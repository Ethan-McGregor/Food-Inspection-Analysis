...
Subway is the largest sandwich company with `r length(unique(subway$Location))` locations with Jimmy Johns trailing behind with `r length(unique(jimmy$Location))` and Quiznos with only `r length(unique(quiznos$Location))`
...
```{r paceholder, echo=FALSE, message=FALSE, warning=FALSE}
m <- count(group_by(subway, Inspection.Type)) %>% filter(Inspection.Type %in% c("Complaint Invest w/Comprehensive","Comprehensive","Food Complaint","Monitoring","Monitoring w/Complaint Investigation"))

inspectionReasonPieChart <- plot_ly(m, labels = ~Inspection.Type, values = ~n, type = 'pie') %>%
